...
function fetchAndDisplayTicket(searchId) {
    const apiUrl = `http://localhost:8888/reservation/view/${searchId}?key=${currUserId}`;
...
function cancelTicket() {
    ...
    if (userConfirmed) {
        const cancelTicketApi = `http://localhost:8888/reservation/delete/${fetchedDataId}?key=${currUserId}`;
